...
| `documentar-avances` | local | local-only | Finalizar tarea; Cerrar sesión con cambios |
...
| `zonix-launch-piloto` | local | local-only | Hitos piloto / calendario Day-D (solo planificación) |
| `zonix-lean-canvas` | local | local-only | — |
| `zonix-legal-contracts-ve` | local | local-only | Revisar contrato / T&C farmacia (checklist) |
| `zonix-onboarding` | local | local-only | Onboarding (incluye pharmacist) |
| `zonix-order-lifecycle` | local | local-only | Estados / flujo de órdenes |
| `zonix-order-tracking-ui` | local | local-only | — |
| `zonix-realtime-events` | local | local-only | Implementar Pusher / FCM |
| `zonix-startup-context` | local | local-only | UI alineada a pack inversor / claims salud |
| `zonix-ui-design` | local | local-only | Diseñar UI/UX Flutter; Crear/modificar pantallas o widgets… |
| `zonix-web-design` | local | local-only | — |
